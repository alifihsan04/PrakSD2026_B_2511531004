...
public class jam_2511531004 {
	...
	// konstruktor 
	public jam_2511531004 (int h , int m , int s ) {
		this.hh = h;
		this.mm = m;
		this.ss = s;
	...
	public static jam_2511531004 fromSeconds(int total ) {
		if ( total < 0 ) throw new IllegalArgumentException("detikn negatif") ;
	...
	// relasional
	public int compareTo( jam_2511531004 other ) {return Integer.compare(this.toSeconds(), other.toSeconds() ); }
	public boolean equals(Object o) {
		if ( ! (o instanceof jam_2511531004 j )) return false;
	...
	// aritmatika 
	public jam_2511531004 plus (jam_2511531004 other) {
		return fromSeconds(this.toSeconds () + other.toSeconds() );
	}
	public jam_2511531004 minus (jam_2511531004 other ) {
		return fromSeconds(Math.floorMod(this.toSeconds () - other.toSeconds(), 24*3600));
	}
	public jam_2511531004 nextSecond () {
		return fromSeconds ( this.toSeconds () + 1);
	}
	public jam_2511531004 nextNSecond (int n ) {
		return fromSeconds (this.toSeconds () + Math.max(0,n) );
	}
	public jam_2511531004 prevSecond () {
		return fromSeconds (Math.floorMod ( this.toSeconds () - 1, 24*3600));
	}
	public jam_2511531004 prevNSecond (int n) {
		return fromSeconds (Math.floorMod(this.toSeconds() - Math.max(0,n) , 24*3600)) ;	
	}
	
	// durasi  (detik --> bisa negatif jika this --> other? sesuai spesifikasi duari 
	public static int durasiDetik(jam_2511531004 jaw, jam_2511531004 jakh) {
